...
<div class="admin-guide">
  <div class="admin-guide-title">관리자가 이 화면에서 해야 할 일</div>
...
def korean_blocker_view(df: pd.DataFrame) -> pd.DataFrame:
    return korean_view(df, {
        ...
        "urgency": "긴급도",
        "status": "상태",
        "founder_decision_required": "관리자 결정 필요",
        "ai_feedback": "AI 피드백",
...
def is_admin() -> bool:
    ...
    return str(user.get("is_admin", "")).lower() in ["true","1","yes","y"] or user.get("role") == "관리자"
...
def find_admin_user_email() -> tuple[str, str]:
    ...
    admins = users_df[
        ...
        (users_df["role"].astype(str) == "관리자")
    ]
    if admins.empty:
        return "", "관리자"
    row = admins.iloc[0]
    return str(row.get("email", "")), str(row.get("name", "관리자"))
...
<div class="quick-message-box">
  ...
  <div class="quick-message-desc">
    막힌 일, 관리자 결정, 질문을 바로 남기세요. 메시지는 팀 메신저에 저장됩니다.
  </div>
</div>
""",
        unsafe_allow_html=True,
    ...
    with st.form("sidebar_quick_messenger_form"):
        ...
        quick_msg = st.text_area("메시지", placeholder="관리자에게 확인받을 내용을 적어주세요.", key="quick_msg_body", height=100)
...
def seed_data():
    ...
    samples = {
        "team_members": pd.DataFrame([
            {"name":"관리자","role":"관리자","area":"전체전략/최종결정","projects":"ARION 전체","contact":"내부","status":"active","today_priority":"수익흐름 결정","memo":"최종 승인자"},
        ...
        "team_daily_actions": pd.DataFrame([
            {"date":today,"owner":"관리자","owner_email":"","area":"전략","project":"ARION Team OS","task":"회원가입/개인업무 버전 테스트","goal":"팀원별 화면 작동 확인","revenue_link":"전체 운영 효율","priority":"high","status":"todo","estimated_minutes":30,"actual_result":"","blocker":"","help_request":"","next_action":"팀원 초대","founder_review":"yes","ai_feedback":"","memo":""},
        ...
        "blockers": pd.DataFrame([
            {"date":today,"owner":"관리자","owner_email":"","project":"사업운영","blocker":"사업용 계좌/카드 안정화 전 결제 기능 보류","needed_help":"결제/정산 기능 제외 유지","urgency":"high","status":"open","founder_decision_required":"yes","ai_feedback":"","resolution_memo":"내부 운영 MVP만 개발"},
        ]),
        "completion_reports": pd.DataFrame([
            {"date":today,"owner":"관리자","owner_email":"","project":"ARION Team OS","completed_task":"회원가입/개인업무 버전 제작","result":"초대코드 기반 회원가입 구조로 전환","proof_link":"","next_task":"팀원 초대","ai_feedback":"","memo":""},
        ]),
...
if st.session_state.user is None:
            ...
            if matched.empty:
                ...
                if verify_password(login_pw, user.get("password_hash","")):
                    ...
                    if account_status == "pending":
                        st.warning("아직 관리자 승인 대기 중입니다. 관리자가 승인하면 로그인할 수 있습니다.")
                    elif account_status in ["rejected", "blocked", "inactive"]:
                        st.error("현재 계정이 승인되지 않았거나 비활성화 상태입니다. 관리자에게 문의하세요.")
                    else:
    ...
    with col_b:
        ...
        with st.form("signup_form"):
            ...
            role = st.selectbox("역할", ["관리자","개발담당","콘텐츠담당","상품담당","운영담당","마케팅담당","기타"])
            ...
            if invite != INVITE_CODE:
                ...
                if not users.empty and cleaned in users["email"].astype(str).str.lower().tolist():
                    ...
                    new_user = pd.DataFrame([{
                        ...
                        "status": "active" if is_first_user or role == "관리자" else "pending",
                        "is_admin": "yes" if role == "관리자" or is_first_user else "no",
                        "created_at": datetime.now().isoformat(timespec="seconds"),
                        "last_login": "",
                    ...
                    if is_first_user or role == "관리자":
                        st.success("관리자 계정 생성 완료. 이제 로그인하세요.")
                    else:
                        st.success("회원가입 신청 완료. 관리자 승인 후 로그인할 수 있습니다.")
...
<div class="approval-box">
  ...
  <div class="approval-desc">
    관리자 계정은 먼저 가입 후 관리자 화면을 사용할 수 있습니다.<br>
    일반 팀원은 회원가입 후 바로 접속되지 않고, 관리자가 승인해야 로그인할 수 있습니다.
  </div>
</div>
""",
        unsafe_allow_html=True,
...
<div class="quick-messenger">
  ...
  <p>업무가 막히거나 관리자 결정이 필요하면 ‘팀 메신저’ 탭에서 <b>관리자에게질문</b> 유형으로 남기면 됩니다.</p>
</div>
""",
        unsafe_allow_html=True,
    )
    ...
    if not my_today.empty:
        ...
        st.info("아직 오늘 배정된 할 일이 없습니다. '내 오늘 할 일' 탭에서 직접 추가하거나 관리자에게 요청하세요.")
...
with tab["오늘 마인드셋"]:
    ...
    with st.form("mindset_form"):
        ...
        support_needed = st.text_area("도움이 필요한 부분", placeholder="예: 관리자 결정 필요, 자료 필요")
...
<div class="team-empty-card">
  ...
  <div class="team-empty-desc">아래에서 오늘 할 일을 직접 추가하거나, 왼쪽 사이드바의 팀 메신저로 관리자에게 업무 배정을 요청하세요.</div>
</div>
""",
            unsafe_allow_html=True,
        )
...
with tab["막힌 일"]:
    ...
    with st.form("blocker_form"):
        ...
        founder_required = st.selectbox("관리자 결정 필요", ["yes","no"])
...
with tab["수익흐름"]:
    ...
    if revenue.empty:
        empty_state(
            "아직 등록된 수익흐름이 없습니다.",
            "관리자가 수익원, 연결 프로젝트, 예상 매출, 다음 행동을 등록하면 팀원들이 현재 사업 방향을 한눈에 이해할 수 있습니다."
        )
    else:
...
if is_admin():
    with tab["관리자 대시보드"]:
        ...
        with st.expander("전체 할 일 고급 편집"):
            ...
            if st.button("관리자: 전체 할 일 저장", key="btn_admin_save_all_actions"):
...
with tab["팀 메신저"]:
    st.subheader("팀 메신저")
    st.caption("팀원 간 질문, 관리자 요청, 프로젝트별 대화를 한 곳에 남깁니다. 실시간 채팅보다는 업무 기록형 메신저입니다.")
    ...
    with st.form("send_message_form"):
        col1, col2 = st.columns(2)
        with col1:
            message_type = st.selectbox("메시지 유형", ["전체공지", "프로젝트방", "1:1메시지", "관리자에게질문"])
